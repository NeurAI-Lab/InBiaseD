...
class ONE_ResNet(nn.Module):
    ...
    def __init__(self, depth, num_classes=1000):
        ...
        self.inplanes = multiplier * 16
        self.conv1 = nn.Conv2d(3, multiplier * 16, kernel_size=3, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(multiplier * 16)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, multiplier * 16, n)
        self.layer2 = self._make_layer(block, multiplier * 32, n, stride=2)

        fix_inplanes=self.inplanes
        self.layer3_1 = self._make_layer(block, multiplier * 64, n, stride=2)
        self.inplanes = fix_inplanes  ##reuse self.inplanes
        self.layer3_2 = self._make_layer(block, multiplier * 64, n, stride=2)
        self.inplanes = fix_inplanes
        self.layer3_3 = self._make_layer(block, multiplier * 64, n, stride=2)
        ...
        self.classfier3_1=nn.Linear(multiplier * 64 * block.expansion, num_classes)
        self.classfier3_2=nn.Linear(multiplier * 64 * block.expansion,num_classes)
        self.classfier3_3=nn.Linear(multiplier * 64 * block.expansion, num_classes)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))
